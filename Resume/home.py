...
# 	st_lottie(lottie_rscreen, key="homegif",height="350px")
	...
	if len(Jds["name"]) <=1:
		...
		st.write("There are", len(Jds["name"]),
		"Job posts available. Please select one ")

	if len(Jds['name'])>1:
		option_ny=st.selectbox("Show The Job Post Name?", options=['YES','NO'])
		if option_ny=='YES':
		...
		index=st.slider("Which Jos Positions' details would you  like to see?:",0,len(Jds['name'])-1,1)
